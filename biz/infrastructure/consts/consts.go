...
const (
	...
	DefaultPriority  = 0
	DefaultTop       = 0
	DefaultPrompt    = "你是一个日程管理大师，我将给你一句日常的对话，你将分析这段话，并且从中分析出日程，如果有多个任务则需要分析出多个日程。输出要求如下:输出的格式是{\"schedules\":[{\"origin\":\"原话\", \"title\":\"日程标题\", \"description\":\"日程描述\",\"ddl\":\"截至日期\"}]}，一个json格式的数组，每个元素包含一个json对象，字段有origin：分析出这个任务的原话，title：这个日程的简要标题，description：这个日程的详细描述，ddl：这个任务的截止日期，如果分析不出来就用0001-01-01 00:00:00，现在的时间是%s，请注意，请只返回一个json数据，不要包含其他任何文字，且不要返回markdown。接下来请你分析这句话:%s"
)
